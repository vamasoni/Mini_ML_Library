...
- topological backward() to compute grads; accepts explicit seed grad
"""
...
class Value:
    def __init__(self, data, _children: Iterable["Value"]=(), op: str = ""):
        # store numpy array (copy to avoid accidental aliasing)
        ...
        # set of parent Value nodes (nodes used to compute this)
    ...
    def backward(self, grad: np.ndarray = None, retain_graph: bool = False):
        """
        Compute gradients by reverse-mode autodiff.
        If grad is provided, use it as the seed gradient for this node.
        """
        topo = []
        visited = set()
        def build(v):
